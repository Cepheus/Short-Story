...
InputListener::InputListener (Scene *scene, Animations * animations,AnimationsAuto * animationsAuto, SceneManager *scmanager, RenderWindow *wnd, Camera *camera) :
        mScene(scene), mAnimations(animations),mAnimationsAuto(animationsAuto), mSceneMgr(scmanager), mWindow(wnd), mCamera(camera), mContinuer(true), mgoUp(false), mgoDown(
